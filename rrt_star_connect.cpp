...
QPair<double, double> RRT_Star_Connect(painter_widget *paintArea, QProgressBar *progress_bar, int max_iter, bool dynamic_drawing, bool stop_when_found, double step){
    //Variable Transfer
    double rad = 50;
    ...
    for (int i = 0; i < max_iter; i++)
    {
        ...
        if (is_valid(paintArea, closest, np, obstacles)){
            ...
            paintArea->add_to_draw("1Green", QColor(0, 255, 0, 255), closest, np);
            closest_pair = T2->find_nearest(np);
            if(distance(np, closest_pair.second) <= 10 && is_valid(paintArea, np, closest_pair.second, obstacles)){
                //paintArea->clear_color("3Red");
                //paintArea->clear_color("3Blue");
                //paintArea->add_to_draw("3Red", QColor(255, 0, 0, 255), np, closest_pair.second );
                time = timer.elapsed();
                cur_path.first = 1;
                ...
                while(point_1 != start){
                    ...
                    //paintArea->add_to_draw("3Red", QColor(255, 0, 0, 255), point_1, point_2);
                ...
                while(point_1 != end){
                    ...
                    //paintArea->add_to_draw("3Blue", QColor(0, 0, 255, 255), point_1, point_2);
                ...
                if(cur_path.first < best_path.first){
                    ...
                    foreach (edge, best_path.second.first) {
                        paintArea->add_to_draw("3Red", QColor(255, 0, 0, 255), edge.first, edge.second);
                    }
                    foreach (edge, best_path.second.second) {
                        paintArea->add_to_draw("3Blue", QColor(0, 0, 255, 255), edge.first, edge.second);
                    }
                }
                if(stop_when_found){
        ...
        if (is_valid(paintArea, closest, np, obstacles)){
            np_id = T2->addNode(np);
            close_points = T2->get_close(np, rad);
            parent = T2->choose_parent(paintArea, np, close_points, obstacles);
            T2->addEdge(parent.first, np_id);
            T2->rewire(paintArea, close_points, QPair<int, Point>(np_id, np), obstacles);
            paintArea->add_to_draw("1GReen", QColor(0, 255, 0, 255), closest, np);
            closest_pair = T1->find_nearest(np);
            if(distance(np, closest_pair.second) <= 10 && is_valid(paintArea, np, closest_pair.second, obstacles)){
                //paintArea->clear_color("3Red");
                //paintArea->clear_color("3Blue");
                //paintArea->add_to_draw("3Blue", QColor(0, 0, 255, 255), np, closest_pair.second);
                time = timer.elapsed();
                cur_path.first = 1;
                cur_path.second.first.clear();
                cur_path.second.second.clear();
